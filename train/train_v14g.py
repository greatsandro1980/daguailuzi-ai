...
def train_vs_random(model, n_games=5000, lr=1e-4):
    """针对随机对手训练"""
    game = Game()
    ...
    for g in range(n_games):
        ...
        if (g + 1) % 500 == 0:
            ...
            if combined > best_combined:
                ...
                best_rule = r1
                best_random = r2
                torch.save(model.state_dict(), '/workspace/projects/rl_v14g_best.pt')
                print(f"    ★ 新最佳!")
...
def main():
    ...
    model, best_rule, best_random = train_vs_random(model, n_games=5000)
    
    print("\n" + "=" * 60)
    print("最终测试 (1000局):")
    
    if os.path.exists('/workspace/projects/rl_v14g_best.pt'):
        model.load_state_dict(torch.load('/workspace/projects/rl_v14g_best.pt', weights_only=True))
