...
def train(model, n_games=6000, lr=3e-5):
    """保守微调：小学习率，少更新轮数"""
    game = Game()
    ...
    for g in range(n_games):
        ...
        if len(memory['states']) >= 1024:
            ...
            surr2 = torch.clamp(ratio, 0.85, 1.15) * advantages  # 更小的裁剪范围
        ...
        if (g + 1) % 500 == 0:
            ...
            if combined > best_combined:
                ...
                best_rule = r1
                best_random = r2
                torch.save(model.state_dict(), '/workspace/projects/rl_v14e_best.pt')
                print(f"    ★ 新最佳!")
...
def main():
    ...
    model, best_rule, best_random = train(model, n_games=6000)
    
    print("\n" + "=" * 60)
    print("最终测试 (1000局):")
    
    if os.path.exists('/workspace/projects/rl_v14e_best.pt'):
        model.load_state_dict(torch.load('/workspace/projects/rl_v14e_best.pt', weights_only=True))
