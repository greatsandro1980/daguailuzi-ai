...
def main():
    ...
    if os.path.exists('/workspace/projects/rl_v14b_best.pt'):
        ...
        results.append(('V14b 策略优化', r1, r2))
        print(f"\nV14b 策略优化 (1000局):")
    ...
    print("对比结果:")
    print("-" * 60)
    print(f"{'模型':<15} {'vs规则':>10} {'vs随机':>10} {'综合':>10}")
    print("-" * 60)
    for name, r1, r2 in results:
        print(f"{name:<15} {r1:>9.1f}% {r2:>9.1f}% {r1+r2:>9.1f}%")
    
    if len(results) >= 2:
        improvement = results[0][1] + results[0][2] - (results[1][1] + results[1][2])
        print(f"\nV14b vs V13: {'+' if improvement > 0 else ''}{improvement:.1f}% 提升")
