...
# ============== 快速训练 ==============
def quick_train(model, n_games=3000):
    game = Game()
    ...
    for g in range(n_games):
        ...
        while True:
            if game.current % 2 == 0:
                ...
                actions = game.get_actions()
                # 动态对手策略：训练早期多用随机Bot，后期多用规则Bot
                if g < n_games * 0.4:
                    random_ratio = 0.9  # 早期90%随机Bot
                elif g < n_games * 0.7:
                    random_ratio = 0.6  # 中期60%随机Bot
                else:
                    random_ratio = 0.3  # 后期30%随机Bot
                
                if np.random.random() < random_ratio:
                    a_idx = np.random.randint(len(actions))  # 随机Bot
                else:
                    a_idx = weak_action(game, error_rate=0.35)  # 弱化规则Bot
                card, cnt = actions[a_idx]
        ...
        if (g + 1) % 500 == 0:
            ...
            if r1 > best_rule:
                best_rule = r1
                torch.save(model.state_dict(), '/workspace/projects/rl_v12_quick_rule.pt')
            if r2 > best_random:
                best_random = r2
                torch.save(model.state_dict(), '/workspace/projects/rl_v12_quick_random.pt')
...
def main():
    ...
    # 快速训练
    model, best_rule, best_random = quick_train(model, n_games=5000)
    ...
    torch.save(model.state_dict(), '/workspace/projects/rl_v12_quick_final.pt')
