...
def main():
    ...
    print("-" * 40)
    avg_rule = np.mean(rule_results)
    avg_random = np.mean(random_results)
    print(f"  平均: vs规则 {avg_rule:.1f}% | vs随机 {avg_random:.1f}%")
    print(f"  综合: {avg_rule + avg_random:.1f}%")
    
    print("\n" + "=" * 60)
    print("目标达成情况:")
    print(f"  vs规则 ≥90%: {'✅' if avg_rule >= 90 else '❌'} ({avg_rule:.1f}%)")
    print(f"  vs随机 ≥90%: {'✅' if avg_random >= 90 else '❌'} ({avg_random:.1f}%)")
