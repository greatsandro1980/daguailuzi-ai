#!/usr/bin/env python3
"""
V15c - 课程学习优化版（与V14b架构兼容）
直接基于V14b权重，使用课程学习优化vs随机
"""
...
# ============== 课程学习训练 ==============
def train_curriculum(model, n_games=3000, lr=5e-5):
    game = Game()
    ...
    print(f"\n课程学习训练 ({n_games}局)...")
    print("=" * 60)
    print("策略: 逐渐增加随机对手比例，优化vs随机表现")
    print("=" * 60)
    
    for g in range(n_games):
        # 课程：随机对手比例逐渐增加
        progress = g / n_games
        random_prob = min(0.9, 0.3 + progress * 0.6)  # 从30%到90%
        ...
        while True:
            ...
            if game.current % 2 == 0:
                ...
                if np.random.random() < random_prob:
                    idx = np.random.randint(len(actions))  # 随机
                else:
        ...
        if len(memory['states']) >= 1024:
            ...
            for _ in range(2):
                ...
                surr2 = torch.clamp(ratio, 0.85, 1.15) * advantages  # 更小的裁剪
                ...
                optimizer.zero_grad()
                loss.backward()
                torch.nn.utils.clip_grad_norm_(model.parameters(), 0.3)  # 更小的裁剪
                optimizer.step()
        ...
        if (g + 1) % 500 == 0:
            ...
            print(f"  {g+1:>5,}局 (随机{random_prob*100:.0f}%) | vs规则: {r1:.1f}% | vs随机: {r2:.1f}% | 综合: {combined:.1f}%")
            
            if combined > best_combined:
                best_combined = combined
                best_rule = r1
                best_random = r2
                torch.save(model.state_dict(), '/workspace/projects/rl_v15c_best.pt')
                print(f"    ★ 新最佳!")
...
def main():
    ...
    # 训练
    model, best_rule, best_random = train_curriculum(model, n_games=6000)
    
    # 最终测试
    print("\n" + "=" * 60)
    print("最终测试 (1000局):")
    
    if os.path.exists('/workspace/projects/rl_v15c_best.pt'):
        model.load_state_dict(torch.load('/workspace/projects/rl_v15c_best.pt', weights_only=True))
