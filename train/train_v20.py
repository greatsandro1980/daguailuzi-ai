...
class Game:
    ...
    def step(self, card, cnt):
        ...
        if team0_done or team1_done:
            ...
            if winner == 0:
                total_reward = reward + strategic_reward + 2.0
            else:
                total_reward = reward + strategic_reward
            return True, winner, total_reward
        
        return False, -1, reward + strategic_reward
...
# ============== 进化策略训练 ==============
def evolve_train(base_model, n_generations=50, pop_size=15, n_test=150):
    print(f"\n进化训练: {n_generations}代, 种群{pop_size}, 测试{n_test}局/评估")
    print("=" * 60)
    ...
    for gen in range(n_generations):
        ...
        for model in population:
            ...
            scores.append((r1 + r2, r1, r2, model))
        ...
        if best[0] > best_combined:
            ...
            best_random = best[2]
            best_model = best[3]
            torch.save(best_model.state_dict(), '/workspace/projects/rl_v20_best.pt')
            marker = "★"
        else:
            marker = ""
...
def main():
    ...
    model, r1, r2 = evolve_train(model, n_generations=50, pop_size=15, n_test=150)
    
    # 最终测试
    print("\n" + "=" * 60)
    print("最终测试 (1000局):")
    
    if os.path.exists('/workspace/projects/rl_v20_best.pt'):
        model.load_state_dict(torch.load('/workspace/projects/rl_v20_best.pt', weights_only=True))
    ...
    if r1 >= 90 and r2 >= 90:
        ...
        print("📊 当前状态:")
        print(f"   {'✓' if r1 >= 90 else '○'} vs规则: {r1:.1f}%")
        print(f"   {'✓' if r2 >= 90 else '○'} vs随机: {r2:.1f}%")
